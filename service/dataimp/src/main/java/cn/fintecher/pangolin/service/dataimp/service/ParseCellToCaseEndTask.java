 ...
 * @Author:peishouwen
 * @Desc:
 * @Date:Create in 17:09 2018/8/2
 */
@Service("parseCellToCaseEndTask")
public class ParseCellToCaseEndTask {
    Logger logger= LoggerFactory.getLogger(ParseCellToCaseEndTask.class);
    ...
    public CompletableFuture<List<String>> cellDataToObject(List<Map<String,String>> sheetDataList , ImportOthersDataExcelRecord record, Map<String ,ImportExcelConfigItem> itemMap,
        ...
        List<CaseEndImportTemp> caseEndImportTempList=new ArrayList<>();
        for (int i = 0; i < sheetDataList.size(); i++) {
            Map<String, String> rowMap = sheetDataList.get(i);
            CaseEndImportTemp caseEndImportTemp = new CaseEndImportTemp();
            String primaryKey = "";
            ...
            for (Map.Entry<String, String> cellMap : rowMap.entrySet()) {
                ...
                if (Objects.isNull(importExcelConfigItem) || StringUtils.isBlank(importExcelConfigItem.getAttribute())) {
                    ...
                    if (importExcelConfigItem.getPropertyType().equals(ExcelAnno.FieldType.CASE.name())) {
                        ...
                        dataimpBaseService.parseCellMap(caseEndImportTemp, cellValue, importExcelConfigItem, errorList, pageNo * pageSize + 1,sheetIndex);
                    }
                }
            }
            caseEndImportTemp.setId(String.valueOf(snowflake.next()));
            ...
            primarykeyMap.clear();
            finalMap.clear();
            caseEndImportTemp.setPrimaryKey(MD5.MD5Encode(primaryKey.concat(caseEndImportTemp.getBatchNumber())));
            caseEndImportTemp.setRemarkMap(remarkMap);
            caseEndImportTemp.setPrincipalId(record.getPrincipalId());
            caseEndImportTemp.setPrincipalName(record.getPrincipalName());
            caseEndImportTemp.setOperator(operatorModel.getUsername());
            caseEndImportTemp.setOperatorTime(ZWDateUtil.getNowDateTime());
            caseEndImportTemp.setOperBatchNumber(record.getOperBatchNumber());
            caseEndImportTempList.add(caseEndImportTemp);
        }
        if(errorList.isEmpty()){
            caseEndImportTempRepository.saveAll(caseEndImportTempList);
        }
