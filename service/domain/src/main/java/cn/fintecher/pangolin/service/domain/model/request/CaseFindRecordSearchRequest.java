 ...
 */
@Data
public class CaseFindRecordSearchRequest extends SearchRequest {
    ...
    public QueryBuilder generateQueryBuilder() {
        ...
        if(Objects.nonNull(this.followTimeStatrt)) {
            qb.must(rangeQuery("findTime").gte(this.followTimeStatrt.getTime()));
        }
        if(Objects.nonNull(this.followTimeEnd)){
            qb.must(rangeQuery("findTime").lte(this.followTimeEnd.getTime()+86400000));
        }
        return qb;
    }
}
